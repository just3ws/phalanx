...
function createPlayerState(id, name, seed) {
...
export function createInitialState(config) {
    ...
    const gameOptions = config.gameOptions ?? { damageMode: 'cumulative' };
    return {
        players: [
...
function setPlayer(state, playerIndex, player) {
 ...
 */
export function deployCard(state, playerIndex, handCardIndex, gridIndex) {
    const player = getPlayer(state, playerIndex);
 ...
 */
export function getDeployTarget(battlefield, column) {
    const frontIdx = column;
    if (battlefield[frontIdx] === null)
        return frontIdx;
    const backIdx = column + 4;
    if (battlefield[backIdx] === null)
        return backIdx;
    return null;
}
/**
 ...
 */
export function advanceBackRow(battlefield, column) {
    const frontIdx = column;
    const backIdx = column + 4;
 ...
 */
export function isColumnFull(battlefield, column) {
 ...
 */
export function getReinforcementTarget(battlefield, column) {
    const backIdx = column + 4;
    if (battlefield[backIdx] === null)
        return backIdx;
    const frontIdx = column;
